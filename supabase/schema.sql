...
create table if not exists public.trades(id uuid primary key default gen_random_uuid(),user_id uuid not null references auth.users(id) on delete cascade,trade_date date not null,exchange text not null,pair text not null,side text not null,position_status text not null default 'Closed' check (position_status in ('Open','Closed')),entry numeric not null,stop_loss numeric not null,take_profit numeric,risk_usd numeric not null,pnl_usd numeric not null,r_multiple numeric not null,setup text,plan_followed boolean default true,mistake text default 'None',notes text,violation boolean default false,created_at timestamptz default now());
