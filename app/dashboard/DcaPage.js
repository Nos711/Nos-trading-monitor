...
const priceNum=n=>Number(n||0).toLocaleString(undefined,{maximumFractionDigits:0});
...
export default function DcaPage({client,user}){
 ...
 return <div className="dcaPage">
  ...
  <div className="dcaLayout"><section className="panel dcaList"><div className="panelHead"><h2>DCA Positions</h2><small>{positions.length}</small></div><div className="body">{positions.length?positions.map(x=>{let z=summary(x,entries.filter(e=>e.position_id===x.id));return <button className={`dcaPosition${selected===x.id?" active":""}${isBtc(x.pair)?" bitcoin":""}`} key={x.id} onClick={()=>setSelected(x.id)}><span><b>{x.pair}</b><small>{x.exchange} · {z.count} entries</small></span><strong>{z.quantity?priceNum(z.breakEven):"—"}</strong></button>}):<div className="emptyState">Create the first position</div>}</div></section>
  <div>{position?<><div className="kpis dcaKpis"><div className="card primaryMetric"><span>AVG BUY PRICE</span><b>{s.quantity?priceNum(s.average):"—"}</b><small>{currency} / COIN</small></div><div className="card primaryMetric"><span>BREAK-EVEN PRICE</span><b>{s.quantity?priceNum(s.breakEven):"—"}</b><small>INCLUDING FEES</small></div><div className="card"><span>COIN HOLDING</span><b>{num(s.quantity)}</b><small>TOTAL QUANTITY</small></div><div className="card"><span>TOTAL INVESTED</span><b>{money(s.total,currency)}</b></div><div className="card"><span>UNREALIZED P&L</span><b className={s.pnl>=0?"pos":"neg"}>{position.current_price?money(s.pnl,currency):"—"}</b></div><div className="card"><span>RETURN</span><b className={s.returnPct>=0?"pos":"neg"}>{position.current_price?s.returnPct.toFixed(2)+"%":"—"}</b></div></div>
  ...
  <section className="panel"><div className="panelHead"><h2>Entry History</h2><small>{s.count} entries</small></div><div className="body table"><table><thead><tr><th>Date</th><th>Entry Price</th><th>Input</th><th>Quantity</th><th>Cost</th><th>Fee</th><th>Running Cost/Coin</th><th>Notes</th><th></th></tr></thead><tbody>{running(position,rows).map(x=><tr key={x.id}><td>{new Date(x.entry_date).toLocaleString()}</td><td>{priceNum(x.entry_price)}</td><td>{num(x.amount)} {x.amount_type}</td><td>{num(x.qty)}</td><td>{money(x.cost,currency)}</td><td>{money(x.fee_usdt,currency)}</td><td><b>{priceNum(x.runningAverage)}</b></td><td>{x.notes||"—"}</td><td><button className="delete" onClick={()=>removeEntry(x.id)}>Delete</button></td></tr>)}{!rows.length&&<tr><td colSpan="9" className="emptyState">No DCA entries yet</td></tr>}</tbody></table></div></section></>:<section className="panel"><div className="emptyState">Select a position to add DCA entries</div></section>}</div></div>
 </div>
}