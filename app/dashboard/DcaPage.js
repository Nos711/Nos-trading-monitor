...
export default function DcaPage({client,user}){
 ...
 return <div className="dcaPage">
  ...
  <section className="panel"><div className="panelHead"><h2>Entry History</h2><small>{s.count} entries</small></div><div className="body table"><table><thead><tr><th>Date</th><th>Entry Price</th><th>Input</th><th>Quantity</th><th>Cost</th><th>Fee</th><th>Running Cost/Coin</th><th>Notes</th><th></th></tr></thead><tbody>{running(position,rows).map(x=><tr key={x.id}><td>{new Date(x.entry_date).toLocaleString()}</td><td>{priceNum(x.entry_price,currency)}</td><td>{num(x.amount)} {x.amount_type}</td><td>{num(x.qty)}</td><td>{money(x.cost,currency)}</td><td>{money(x.fee_usdt,currency)}</td><td><b>{priceNum(x.runningAverage,currency)}</b></td><td>{x.notes||"—"}</td><td><button className="delete" onClick={()=>removeEntry(x.id)}>Delete</button></td></tr>)}{!rows.length&&<tr><td colSpan="9" className="emptyState">No DCA entries yet</td></tr>}</tbody></table></div></section></>:<section className="panel"><div className="emptyState">Select a position to add DCA entries</div></section>}</div></div>
 </div>
}